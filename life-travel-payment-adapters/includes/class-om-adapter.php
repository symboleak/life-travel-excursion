<?php
/**
 * Adaptateur pour la passerelle Orange Money de IwomiPay
 */
...
if (!class_exists('WC_OM_Iwomipay')) {
    return;
}

/**
 * Classe adaptateur pour Orange Money
 */
class LTE_OM_Adapter extends WC_OM_Iwomipay {
    /**
     * Constructeur
     */
    public function __construct() {
        ...
        $this->id = 'iwomipay_payment_om';
        $this->method_title = __('Orange Money (Life Travel)', 'life-travel-payment-adapters');
        $this->method_description = __('Paiement via Orange Money - Adapté pour Life Travel', 'life-travel-payment-adapters');
        
        // Ajouter des hooks spécifiques à Life Travel
        add_action('woocommerce_receipt_' . $this->id, array($this, 'lte_receipt_page'));
        add_action('woocommerce_api_lte_om_callback', array($this, 'lte_check_response'));
        
        // Modifier l'URL de callback
        $this->notify_url = WC()->api_request_url('lte_om_callback');
    }
    ...
    public function init_settings() {
        ...
        $this->settings['description'] = isset($this->settings['description']) 
            ...
            : __('Paiement sécurisé via Orange Money - Par Life Travel', 'life-travel-payment-adapters');
    }
    ...
    public function lte_receipt_page($order_id) {
        ...
        echo __('Vous allez être redirigé vers Orange Money pour compléter votre paiement...', 'life-travel-payment-adapters');
        echo '</div>';
    ...
    private function log_payment_response($response_data) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG === true) {
            $log_file = WP_CONTENT_DIR . '/lte-payment-logs/om-' . date('Y-m-d') . '.log';
    ...
    public function process_payment($order_id) {
        ...
        do_action('lte_before_om_payment_process', $order_id, $this);
        
        // Déléguer à la méthode parente
        $result = parent::process_payment($order_id);
        
        // Ajouter une action de post-traitement
        do_action('lte_after_om_payment_process', $order_id, $result, $this);
